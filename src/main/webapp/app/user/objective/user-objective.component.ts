...
export default defineComponent({
  ...
  setup() {
    ...
    const retrieveObjectives = async () => {
    ...
    onMounted(async () => {
      await retrieveObjectives();
    });

    const removeId: Ref<number> = ref(null);
    const removeEntity = ref<any>(null);
    const prepareRemove = (instance: IObjective) => {
    ...
    const removeObjective = async () => {
      try {
        await objectiveService().delete(removeId.value);
        const message = t$('improvemyselfApp.objective.deleted', { param: removeId.value }).toString();
