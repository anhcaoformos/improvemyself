<template>
  <div>
    <h2 id="page-heading" data-cy="GoalHeading">
      <span v-text="t$('improvemyselfApp.goal.home.title')" id="goal-heading"></span>
    ...
    <div class="alert alert-warning" v-if="!isFetching && goals && goals.length === 0">
      <span v-text="t$('improvemyselfApp.goal.home.notFound')"></span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('title')">
              <span v-text="t$('improvemyselfApp.goal.title')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'title'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('description')">
              <span v-text="t$('improvemyselfApp.goal.description')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'description'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('priority')">
              <span v-text="t$('improvemyselfApp.goal.priority')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('ledger.id')">
              <span v-text="t$('improvemyselfApp.goal.ledger')"></span>
          ...
          <tr v-for="goal in goals" :key="goal.id" data-cy="entityTable">
            ...
            <td>
              <div v-if="goal.ledger">
                <router-link :to="{ name: 'LedgerView', params: { ledgerId: goal.ledger.id } }">{{ goal.ledger.id }}</router-link>
              </div>
            </td>
            <td class="text-right">
              <div class="btn-group">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="improvemyselfApp.goal.delete.question" data-cy="goalDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-goal-heading" v-text="t$('improvemyselfApp.goal.delete.question', { id: removeId })"></p>
      </div>
